...
def open_slice(args, slice_id, cell_type):
    array_point = []
    file_path = build_path_histo(args.mri_projections_dir, slice_id, args.full_annotations_mask)
...
def bin_mri_coord(args, slices_indices, list_arrays, ref_nifti):
    ...
    full_list_points = []
    weights = []
    for index, sparse_array in zip(slices_indices, list_arrays):
...
def main(args):
    ...
    for cell_type in args.cell_types:
        list_slice = [open_slice(args, slice_id, cell_type) for slice_id in slices_indices]
        ...
        out_volume_name = os.path.join(
            os.path.dirname(args.nifti_reference),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scale_y", type=int, default=None)
