...
def normalization(x):
    # function to be adapted and tested, but we should get the entire slide and not just the current patch !
    # get the lowest resolution
    lowest = x[-1].reshape((-1, 3))

    # get extremas after suppression of white and black for each channel
    r_, g_, b_ = lowest[:, 0], lowest[:, 1], lowest[:, 2]
    r = r_[(r_ != 0) & (r_ != 255)]
    g = g_[(g_ != 0) & (g_ != 255)]
    b = b_[(b_ != 0) & (b_ != 255)]

    if min(len(r), len(g), len(b)) == 0:
        return x

    low_r = np.percentile(r, 5)
    high_r = np.percentile(r, 95)
    low_g = np.percentile(g, 5)
    high_g = np.percentile(g, 95)
    low_b = np.percentile(b, 5)
    high_b = np.percentile(b, 95)

    # renormalize

    delta_r = high_r - low_r
    delta_g = high_g - low_g
    delta_b = high_b - low_b

    low = np.array([low_r, low_g, low_b])
    delta = np.array([delta_r, delta_g, delta_b])
    max_value = 255.

    new_x = [
        np.clip((y - low) * (max_value / delta), 0, 255).astype(int).astype(np.uint8)
        for y in x
    ]
...
def normalization2(x):
    ...
    valid_pixels = np.logical_and(
        np.any(lowest != [0, 0, 0], axis=-1),
        np.any(lowest != [255, 255, 255], axis=-1),
    )

    pixels = lowest[valid_pixels].flatten()

    if len(pixels) == 0:
        return x

    low_r = np.percentile(pixels, 5)
    high_r = np.percentile(pixels, 95)
    ...
    new_x = [
        np.clip((y - low_r) * (max_value / delta_r), 0, 255).astype(int).astype(np.uint8)
        for y in x
    ]
...
def normalization3(x):
    ...
    new_x = [
        np.clip(y * (max_value / high_r), 0, 255).astype(int).astype(np.uint8)
        for y in x
    ]
...
def train(model_full, model_red, train_gen, test_gen, test_gen_red, starts_from, max_epochs, root_output="./default"):
    for i in range(starts_from, max_epochs):
        ...
        x_train, _ = train_gen[0]
        show_images([z for im in x_train for z in im])
        plt.savefig(f"{root_output}_train_e{i}.png")
